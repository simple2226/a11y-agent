...
def explain(error: ClientError) -> str:
    ...
    if "create a support case" in lowered or "not authorized to perform this action" in lowered:
        return ("account NOT AUTHORISED for this provider. File an Account-and-billing "
                "support case, and use a different provider meanwhile.")
    if "operation not allowed" in lowered:
        return "Operation not allowed (see verdict below -- provider gate or account block)"
    if "toolchoice" in lowered or "tool choice" in lowered:
        return f"model does not support this toolChoice mode ({message[:120]})"
    if code == "AccessDeniedException":
        return "IAM: principal lacks bedrock:InvokeModel / bedrock:Converse"
...
def main() -> int:
    ...
    print("\nVERDICT:")
    if len(blocked_providers) >= 2:
        print("  'Operation not allowed' across MULTIPLE providers "
              f"({', '.join(sorted(blocked_providers))}).")
        print("  This is an ACCOUNT-LEVEL Bedrock block, not a per-provider gate.")
        print("  No model id or region will fix it. Options:")
        print("    1. Support case -> Account and billing (free on Basic support).")
        print("    2. Use a different AWS account that already has Bedrock working.")
        print("    3. Switch the model off Bedrock and keep the rest of the stack on AWS:")
        print('         $env:MODEL_PROVIDER="gemini";  $env:GEMINI_API_KEY="..."')
    elif blocked_providers == {"anthropic"}:
        print("  Only Anthropic is blocked -> submit the one-time usage form in the")
        print("  Bedrock console playground, or use an Amazon Nova model instead.")
    else:
        print("  No model id worked. Read the per-model reasons above.")

    print("\n  Offline fallback that needs no credentials at all:")
    print('    $env:A11Y_MOCK_MODEL="1"')
    return 1
