...
# Rules where the accessible name is nearly always recoverable from the element's
# own attributes or text. Deferring these is a mistake, not caution.
DERIVABLE_NAME_RULES = {
    "label",
    "select-name",
    "button-name",
    "link-name",
...
# How many failing nodes the model sees per call. Every node costs tokens, so
# this is not "send everything" -- but 12 was too few to spot the shared class
# that makes a grouped selector possible, which is what clearing a large rule
# depends on. Repair passes get a FRESH 20 each time, so a rule failing on 60
# nodes is reachable across passes.
MAX_NODES_SENT_PER_CLUSTER = 20
