...
# Never editable by any operation.
PROTECTED_SELECTORS = {"*", ":root", "script", "style"}

# Editable by attribute operations only. html/head/body legitimately need
# attribute fixes -- html-has-lang is unfixable without setting lang on <html> --
# but replacing their text or wrapping them destroys the document.
STRUCTURAL_ROOTS = {"html", "head", "body"}

ATTRIBUTE_ONLY_OPS = {"set_attribute", "remove_attribute"}

# If one selector matches more nodes than this, we reject the edit rather than
# apply a blind sweep across the page.
# A grouped selector is the only way to clear a rule that fails on dozens of
# nodes, and the whole point of the coverage guidance in prompts.py. A cap of 40
# rejected exactly the edits we now ask for. This is still a guard against a
# selector so broad it rewrites the page -- it is just set above the size of a
# real navigation or card grid rather than below it.
MAX_MATCHES_PER_EDIT = 150
...
def validate_edit(edit: dict) -> None:
    ...
    normalised = selector.lower()
    if normalised in PROTECTED_SELECTORS:
        raise InvalidEdit(f"selector '{selector}' is protected")

    if normalised in STRUCTURAL_ROOTS and edit["op"] not in ATTRIBUTE_ONLY_OPS:
        raise InvalidEdit(
            f"'{selector}' accepts only {' / '.join(sorted(ATTRIBUTE_ONLY_OPS))}, "
            f"not '{edit['op']}'"
        )
