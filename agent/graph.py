...
The agent loop.
    ...
    mirror -> audit_original -> baseline -> cluster -> [ per cluster:
                                                   generate -> apply -> verify
                                                   -> (repair, while improving) ]
           -> audit_final -> finalise

`baseline` is the deterministic pass: every fix that has exactly one right
answer is applied before a model is called at all. It is what stops a page
coming back unchanged when the model is weak, rate-limited or simply cannot see
enough of the page. See agent/deterministic.py.
...
from agent.deterministic import deterministic_edits
...
class RunState(TypedDict):
    ...
    # What is left for the MODEL to work on. The deterministic pass runs first
    # and removes everything it could fix without one, so the model's budget
    # goes on the genuinely ambiguous nodes. original_violations stays the true
    # "before" for scoring.
...
def node_audit_original(state: RunState) -> StateUpdate:
    ...
    return {
        ...
        "violations_to_fix": result.violations,
...
def node_baseline(state: RunState) -> StateUpdate:
    """Apply every mechanically-derivable fix before the model is involved.

    This is what stops a page coming back completely unchanged. Missing lang
    attributes, blocked zoom, icon links whose name is already sitting in a
    title or a nested alt, and low-contrast text whose colours axe has already
    measured -- none of those need a model, and on a real site they are the
    majority of the violations.

    Held to the same standard as a model pass: if it introduces anything or
    fails to improve the score, the whole baseline is reverted.
    """
    edits = deterministic_edits(state["original_violations"])
    if not edits:
        return {"log": ["baseline: nothing mechanically fixable"]}

    applied = apply_edits(state["original_html"], edits)
    if not applied.applied:
        return {
            "log": [
                f"baseline: {len(edits)} edit(s) proposed, none applied "
                f"({len(applied.rejected)} rejected)"
            ]
        }
    ...
    if delta.introduced_rules or delta.score_after < delta.score_before:
        return {
            "log": [
                f"baseline: reverted -- score {delta.score_before}->{delta.score_after}"
                + (f", introduced {', '.join(delta.introduced_rules)}"
                   if delta.introduced_rules else "")
            ]
        }

    def with_snippets(entry) -> dict:
        return {
            **entry.edit,
            "matched_nodes": entry.matched_nodes,
            "before_snippets": entry.before_snippets,
            "after_snippets": entry.after_snippets,
        }

    kept = [with_snippets(entry) for entry in applied.applied]
    return {
        "working_html": applied.html,
        "violations_to_fix": result.violations,
        "score_after": delta.score_after,
        "accepted_edits": state["accepted_edits"] + kept,
        "applied_signatures": state["applied_signatures"]
        + [edit_signature(entry.edit) for entry in applied.applied],
        "log": [
            f"baseline: {len(applied.applied)} deterministic edit(s) applied, "
            f"no model call, score {delta.score_before}->{delta.score_after}"
        ],
    }


def node_cluster(state: RunState) -> StateUpdate:
    clusters = build_clusters(state["violations_to_fix"])[:MAX_CLUSTERS_PER_RUN]
...
def build_graph():
    ...
    graph.add_edge("audit_original", "baseline")
    graph.add_edge("baseline", "cluster")
