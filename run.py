...
def run_agent(html_text: str, page_url: str, output_directory: Path) -> dict:
    ...
    report = {
        ...
        "deferred_rules": final_state.get("deferred_rules", []),
        "reverted_rules": [
            report["rule"]
            for report in final_state.get("cluster_reports", [])
    ...
    print(f"  deferred to human: {len(report['deferred_items'])} "
          f"across rules {', '.join(report['deferred_rules']) or '(none)'}")
    print(f"  edits applied:     {len(report['accepted_edits'])}")
    if report["reverted_rules"]:
        print(f"  REVERTED:          {', '.join(report['reverted_rules'])} "
              f"(made the page worse)")
...
def apply_provider_arguments(arguments) -> None:
    """Translate CLI flags into the environment the provider layer reads.

    Must run before the first request_edits call. resolve_provider() reads the
    environment on every call, so this takes effect even though the module was
    imported earlier.
    """
    if arguments.provider:
        os.environ["MODEL_PROVIDER"] = arguments.provider
        # An explicit --provider beats a stale A11Y_MOCK_MODEL in the shell.
    ...
    if arguments.model:
        ...
        os.environ[variable] = arguments.model
